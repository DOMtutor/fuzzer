...
from repository.repository import RepositoryProblem, ProblemRepository, RepositoryConfiguration
...
def get_problem_seeds(problem_name):
    secret_path = config.problems.load_problem(problem_name).directory / "data" / "secret"
...
def start_fuzzing():
    ...
    state[fuzzing_id] = FuzzingThread(fuzzing_id, app.logger, data, config.problems)
